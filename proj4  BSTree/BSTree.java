...
class BSTree<T extends Comparable<T>>
{
	...
	public boolean contains( T key , BSTNode<T> bstNode) 
	{
		Comparable cmpData = (Comparable) key;
		
		if(key.compareTo(bstNode.getKey()) > 0) {
			if(bstNode.getRight() == null)
				return false;
			return contains(key, bstNode.getRight());
		} 

		else if(key.compareTo(bstNode.getKey()) < 0) {
			if(bstNode.getLeft() == null)
				return false;
			return contains(key, bstNode.getLeft());
		}
