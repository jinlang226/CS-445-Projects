...
class BSTreeL6<T>
{
  ...
  private void calcLevelCounts( BSTNode root, int levelCounts[], int level )
  {
	...
	{
		//just traverse
		//reach any node n ~ nlog(n)
		//insert in to array -> 1
		//sort array nlog(n)
		...
		balancedBST.root = BalanceBSTHelper(keys, 0, keys.size()-1);
	...
	private void CopyInOrderHelper(ArrayList<T> arr, BSTNode<T> root) 
	{
		if (root == null) return;
		CopyInOrderHelper(arr, root.left);
		arr.add(root.key);
		CopyInOrderHelper(arr, root.right);
	}

	private BSTNode<T> BalanceBSTHelper(ArrayList<T> arr, int start, int end)
	{
		if(start > end) {
			return null;
		}
		...
		int mid = (start + end) / 2;
		BSTNode<T> newRoot =  new BSTNode<T>(arr.get(mid), null, null);
		newRoot.left = BalanceBSTHelper(arr, start, mid - 1);
		newRoot.right = BalanceBSTHelper(arr, mid + 1, end);
	...
	{	
		if(equals(root, other.root) == true) {
			return true;
		}
		return false;
	...
	{	
		...
		//both non-empty
		if (thisRoot != null && otherRoot != null)  
            return (thisRoot.key == otherRoot.key 
